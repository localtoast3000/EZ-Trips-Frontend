...
export default ({ onSelectDateRange, responseFormat, maxDate, minDate }) => {
  ...
  const returnSelectedRange = (fd, ld) => {
    ...
    if (responseFormat) {
      onSelectDateRange({
        firstDate: isWrongSide ? ld.format(responseFormat) : fd.format(responseFormat),
        secondDate: isWrongSide ? fd.format(responseFormat) : ld.format(responseFormat),
      });
    } else {
      onSelectDateRange({
